...
def pathFind(the_map, dirs, dx, dy , xA, yA, xB, yB,  n=MAX_WIDTH, m=MAX_HEIGHT):
    ...
    while len(pq[pqi]) > 0:
        ...
        if x == xB and y == yB:
            #print("dir_maps",dir_map)
            #print("heap", len(pq), len(pq[0]))
    ...
    return ['NOOOOO'] # if no route found
...
def ellers_maze(option, width=MAX_WIDTH, height=MAX_HEIGHT):
	...
	sets = list(range(len(maze[0])))#range(len(colsize))
	for y, row in enumerate(maze):
		for x, col in enumerate(row[:-1]):#not taking the last column
...
def make_maze_recursive_call(maze, top, bottom, left, right):
	...
	if top > y + 3 and x > left + 3:
		make_maze_recursive_call(maze, top, y, left, x)#2

	if top > y + 3 and x + 3 < right:
		make_maze_recursive_call(maze, top, y, x, right)#1

	if bottom + 3 < y and x + 3 < right:
		make_maze_recursive_call(maze, y, bottom, x, right)#4

	if bottom + 3 < y and x > left + 3:
		make_maze_recursive_call(maze, y, bottom, left, x)#3
